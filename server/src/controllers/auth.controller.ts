...
export const register = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    let existingUser = inMemoryUsers.find(u => u.email === email.toLowerCase());
    
    // If not in memory and MongoDB is available, try database
    if (!existingUser && isMongoConnected()) {
      try {
        existingUser = await User.findOne({ email: email.toLowerCase() });
      } catch (error) {
        console.log("MongoDB query failed, skipping DB check");
      }
    }
    ...
    if (isMongoConnected()) {
      try {
        ...
        console.log("MongoDB create failed, saving user to in-memory storage");
      }
    }
